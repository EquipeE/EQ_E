...
if (!file_exists("../img/posts/{$argv[2]}"))
...
foreach ($res as $r)
	$cardsText .= '<div class="card">' . '<a href="./html/post.html?id=' . $r['id'] . '"><img src="' . './img/posts/' . $r['imagem'] . '"></a><div class="card-text">' . $r['titulo'] . '</div></div>' . "\n";

array_pop($res); /* Carrossel usa apenas 3 posts */

foreach ($res as $r)
	$carrosselText .= '<div class="slide">' . '<a href="./html/post.html?id=' . $r['id'] . '"><img src="' . './img/posts/' . $r['imagem'] . '"></a></div>' . "\n";

$template = file_get_contents("../html/index_template.html");
$final = str_replace("!!!CARROSSEL", $carrosselText, str_replace("!!!CARDS", $cardsText, $template));

if (!file_put_contents("../index.html", $final))
